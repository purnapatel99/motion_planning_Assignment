...
class PRM:
    ...
    def uniform_sample(self, n_pts):
        ...
        row_col_ratio = self.size_row / self.size_col
        n_col = math.sqrt(n_pts / row_col_ratio)
        n_row = n_pts / n_col
        row = 0
        col = 0
        for i in range(0, int(n_col)+1):
            row = 0
            col = i * self.size_col / n_col
            for j in range(0, int(n_row)+1):
                row = j * self.size_row / n_row
                if self.map_array[int(row)][int(col)] == 1:
    ...
    def gaussian_sample(self, n_pts):
        ...
        while i < n_pts:
            ...
            col = random.randint(0, self.size_col-1)
            p1 = (row, col)
            p2 = random.normal(loc=p1, scale=10)
            p2 = (int(p2[0]), int(p2[1]))
            if p2[0] in range(0, self.size_row-1) and p2[1] in range(0, self.size_col-1):
                if self.map_array[p1[0]][p1[1]] == 1 and self.map_array[p2[0]][p2[1]] == 0:
                    self.samples.append(p1)
                elif self.map_array[p1[0]][p1[1]] == 0 and self.map_array[p2[0]][p2[1]] == 1:
    ...
    def bridge_sample(self, n_pts):
        ...
        while i < n_pts:
            ...
            col = random.randint(0, self.size_col-1)
            p1 = (row, col)
            if self.map_array[p1[0]][p1[1]] == 0:
                p2 = random.normal(loc=p1, scale=15)
                p2 = (int(p2[0]), int(p2[1]))

                if p2[0] in range(0, self.size_row - 1) and p2[1] in range(0, self.size_col - 1):
                    if self.map_array[p2[0]][p2[1]] == 0:
                        mid_row = (p1[0] + p2[0]) / 2
                        mid_col = (p1[1] + p2[1]) / 2
                        if self.map_array[int(mid_row)][int(mid_col)] == 1:
    ...
    def sample(self, n_pts=1000, sampling_method="uniform"):
        ...
        if sampling_method == "uniform":
            ...
            r = 17
            self.k_neighnours = 10
        elif sampling_method == "gaussian":
            self.gaussian_sample(n_pts)
            r = 25
            self.k_neighnours = 50
        ...
        pairs1 = self.kdtree.query_pairs(r)
        pairs1 = list(pairs1)
        pairs = []
        for i in pairs1:
            if self.check_collision(self.samples[i[0]], self.samples[i[1]]):
                pairs.append((i[0], i[1], self.dis(self.samples[i[0]], self.samples[i[1]])))
    ...
    def search(self, start, goal):
        ...
        start_dist, start_ID = self.kdtree.query(start, k=self.k_neighnours)
        goal_dist, goal_ID = self.kdtree.query(goal, k=self.k_neighnours)
        ...
        for i in goal_ID:
            if self.check_collision(goal, self.samples[i]):
                start_pairs.append(('goal', i, self.dis(goal, self.samples[i])))

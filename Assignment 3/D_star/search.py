...
class DStar:
    ...
    def process_state(self):
        ...
        for i in neighbours:
            if i.tag == "NEW" or i.tag == "OPEN":
                cost = self.cost(current_node, i) + current_node.k
                self.insert(i, cost)
                # print("Not closed")
                if i.k == i.h and current_node.parent != i:
    ...
    def modify_cost(self, obstacle_node, neighbor):
        ...
        if neighbor.parent == obstacle_node:
            neighbor.parent = None
            cost = self.cost(obstacle_node, neighbor)
            self.insert(neighbor, cost)
        elif obstacle_node.parent == neighbor:
            neighbor.tag = "OPEN"
    ...
    def prepare_repair(self, node):
        ...
        for i in neighbours:
            if i.is_dy_obs is True and i.is_obs is False:
                i.is_obs = True
                neigh_neigh = self.get_neighbors(i)
                for j in neigh_neigh:
                    k_min = self.modify_cost(i, j)
                    # print(k_min)
    ...
    def run(self):
        ...
        len_row = len(self.grid_node)
        len_col = len(self.grid_node[0])


        while len(self.path) > 1:
            for row in range(len_row):
                for col in range(len_col):
                    self.grid_node[row][col].tag = "NEW"

            self.open = set()
            current_node = self.path.pop(1)
